...
## Phase 5 — RAW / AI 마스크 / 패키징

### RAW (✅ 동작 확인: DNG)
- ✅ **구현 방식**: (1) `libraw-wasm`(LibRaw의 WASM 빌드, 자체 Worker)으로 실제 디코딩 → 카메라 화이트밸런스·sRGB·8비트로 **현상한 무손실 PNG**를 편집 기준본으로 사용. (2) LibRaw가 실패하면 RAW에 들어 있는 **내장 JPEG 프리뷰 중 가장 큰 것**으로 폴백(마커 파싱, 단위 테스트). 어느 방식이 쓰였는지는 메타데이터 패널의 "RAW 현상"에 표시
- ✅ 지원 확장자: CR2/CR3/CRW/NEF/NRW/ARW/SR2/DNG/RAF/ORF/RW2/PEF/SRW/X3F/3FR/ERF/MRW/KDC/DCR/IIQ (실제 디코딩 가능 여부는 LibRaw가 결정)
- ✅ 원본 RAW는 그대로 보관하고 편집·내보내기는 현상본 기준. 새로고침 후에도 유지
- ⚠ 검증은 **작은 DNG 1개**(736×1104, 1.6초)로만 했음. 실제 카메라 RAW(CR3/NEF/ARW 등)와 대용량(24MP+)은 직접 확인하지 못함
- ⚠ **한계**: 8비트 sRGB로 미리 현상하므로 RAW의 넓은 다이내믹 레인지·화이트밸런스 자유도(하이라이트 복구, 색온도 재조정)는 제한됨. 16비트 선형 파이프라인은 미구현
- ⚠ LibRaw는 LGPL/CDDL 라이선스(WASM 빌드 래퍼는 ISC). 배포 시 고지 필요


### AI 마스크 (✅ 동작 확인)
- ✅ **피사체(AI)**: U²-Net-P(Apache-2.0, 4.5MB ONNX)를 `onnxruntime-web`(WASM, 단일 스레드, 전용 Worker)로 브라우저 안에서 실행. 인물 사진에서 얼굴·머리카락·옷을 정확히 분리함을 확인. **사진은 서버로 전송되지 않음**
- ✅ **하늘**: 신경망이 아니라 **색·위치·연결성 휴리스틱**(파란색/밝은 무채색 + 매끈함 + 프레임 위쪽과 연결). 맑은 하늘/흐린 하늘에 적합, 노을·복잡한 하늘은 브러시로 다듬어야 할 수 있음. 단위 테스트 포함
- ✅ 결과는 브러시 구성 요소의 기본 비트맵으로 저장 → **브러시로 추가/지우기 가능**, 추가/빼기/교차/반전·마스크별 보정 그대로 사용, 내보내기에도 반영
- ⚠ 첫 사용 시 WASM(약 14MB, gzip 3.7MB) + 모델(4.5MB)을 내려받아 10초 안팎 소요(이후 캐시). 처리는 CPU라 큰 이미지도 1024px 축소본 기준(320×320 추론)이라 경계가 거칠 수 있음
- ❌ 사람 부위(얼굴/머리카락/피부) 세분화, SAM 계열 클릭 선택: 미구현(경량 모델 없음)

### 성능 / 패키징
- ✅ **PWA**: `manifest.webmanifest` + 서비스 워커(앱 셸 네트워크 우선·해시 자산 캐시 우선). 설치 가능, **오프라인 로드 확인**(서버 종료 후 새로고침해도 앱이 뜸). 사진·편집값은 IndexedDB/OPFS에 있어 서비스 워커를 거치지 않음. ⚠ RAW(libraw)·AI(onnx) WASM은 **한 번 온라인에서 사용한 뒤에** 캐시되어 오프라인에서 동작
- ✅ **필름스트립 가상화**(300장 가져오기 시 DOM 항목 14개), Library 그리드 가상화, 원본 ImageBitmap·텍스처는 사용 즉시 해제, 300장 일괄 가져오기(썸네일·EXIF·저장) 오류 0
- ✅ 원본 해상도 내보내기는 GPU 최대 텍스처 크기 기준 타일 렌더링(Phase 3)
- ❌ **Tauri 패키징: 미구현** — PWA를 택함. 브라우저 전용 API는 `platform/`(파일 접근, 원본 저장, 다운로드, 기능 감지)에 모여 있어 교체 지점은 준비돼 있으나, 네이티브 셸(src-tauri)은 만들지 않았음
- ⚠ 50MP+ 이미지는 원본 해상도 밉맵 텍스처 1장(약 260MB VRAM)을 사용하므로 저사양 GPU에서는 느리거나 실패할 수 있음(최대 텍스처 크기 초과 시 자동 축소는 됨). 실제 50MP 사진으로는 검증하지 못함
- 서드파티 라이선스: `THIRD_PARTY.md` (LibRaw LGPL/CDDL, U²-Net Apache-2.0 등)

### EXIF 프레임 (✅ 동작 확인)
- ✅ Export 패널에 "EXIF Frame" 섹션 추가: 사진 아래/위에 카메라·렌즈·초점거리·노출값(조리개/셔터/ISO)·촬영일·자유 텍스트를 캡션 바로 표시. 원본 픽셀은 그대로 두고 캔버스만 확장해서 그림(리사이즈·워터마크 다음 단계, 타일 렌더링·ZIP 일괄 내보내기에도 동일하게 적용)
- ✅ 스타일 3종: Minimal(한 줄), Strap(두 줄 + 구분선), Film(사방 여백 있는 폴라로이드 느낌). 위치(위/아래), 배경(밝게/어둡게) 선택
- ✅ 항목별 표시 토글(카메라/렌즈/초점거리/노출값/촬영일) + 자유 텍스트. 선택한 사진의 실제 EXIF로 실시간 미리보기 표시, 아무 항목도 없으면 경고 후 프레임을 그리지 않음
- ✅ 글자 크기·바 높이는 출력 해상도에 비례(단위 테스트로 검증), 배경/글자색 팔레트는 직접 디자인(참고 사이트의 UI·로고를 그대로 베끼지 않음)
- ⚠ 커스텀 폰트 업로드, 아이콘(카메라 브랜드 로고), 템플릿 토큰 직접 편집(`{ISO}{F}` 등)은 미구현 — 체크박스 조합으로 대체

### Strap 스타일 고정 레이아웃 + 렌즈 직접 입력 (✅ 동작 확인)
- ✅ **Strap 스타일을 고정 2단 구성으로 재설계**: 왼쪽 "ISO### F# 셔터속도"(굵게) + 전체 날짜시간 "YYYY/MM/DD HH:MM:SS"(연하게), 오른쪽 로고 + 세로 구분선 + 카메라(굵게) + 렌즈(연하게, 오른쪽 정렬). 항목을 꺼도 레이아웃 구조는 유지되고 해당 줄만 비워짐
- ✅ **렌즈 이름 직접 입력** 필드 추가: 비워두면 EXIF 렌즈명을 쓰고, 입력하면 그 값이 모든 스타일에서 우선 사용됨(EXIF 렌즈명이 비어있거나 부정확한 경우 대비)
- ✅ `showDate` 기본값을 켜짐으로 변경(Strap의 기본 모습이 참고 레이아웃과 바로 일치하도록)
